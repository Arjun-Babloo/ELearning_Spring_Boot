...
import com.cts.exception.CourseException;
import com.cts.exception.CourseNotFoundException;
import com.cts.exception.UnknownException;
...
public class CourseService {
	...
	public Course createCourse(Course course) {
		...
		if(user.equals(null)) {
			throw new UnknownException("User Field May Null here. check if user is exists.");
		}
		
		if(user.getRole().equals("STUDENT")) {
			throw new CourseException("Course creation only made by instructor, not by student");
		}
		
		else if (!iCourseRepository.existsByTitle(course.getTitle())) {
